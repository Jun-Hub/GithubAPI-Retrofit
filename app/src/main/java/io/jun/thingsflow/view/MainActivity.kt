...
class MainActivity : AppCompatActivity() {
    ...
    private fun searchGithub(org:String, repo:String) {
        ...
        service.fetchIssue(org, repo).enqueue(object : Callback<List<GithubIssue>> {
            override fun onFailure(call: Call<List<GithubIssue>>, t: Throwable) {
                Log.e("fail on getIssue", "msg : ${t.message}")
                Toast.makeText(this@MainActivity, t.message, Toast.LENGTH_LONG).show()
            }

            override fun onResponse(call: Call<List<GithubIssue>>, response: Response<List<GithubIssue>>) {
                // 요청에 성공한 경우
                response.body()?.let { issueList ->
                    if(issueList.isNotEmpty()) {
                        issueAdapter.setIssue(issueList)
                        search_textView.text = String.format(getString(R.string.searched_issue), org, repo)
                    }
